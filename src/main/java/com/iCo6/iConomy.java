...
public class iConomy extends JavaPlugin {
     ...
     * @return String
     */
    public static String format(UUID account) {
            ...
            try {
                ...
                if (Database.isSQL()) {
                    if (!Database.tableExists(Constants.Nodes.DatabaseTable.toString())) {
                        System.out.println("+-------------------------------------------------+");
                        System.out.println("Type is null? " + (Database.getType() == null));
                        String SQL = Common.resourceToString("Create-Table-" + Database.getType().toString().toLowerCase() + ".sql");
                        System.out.println("SQL null? " + (SQL == null));
                        System.out.println("+-------------------------------------------------+");
                    ...
                    try {
                        run.query(old, "SELECT * FROM " + table, new ResultSetHandler() {
                            public Object handle(ResultSet rs) throws SQLException {
                                ...
                                if (next)
                                    if (iConomy.Accounts.exists(UUID.fromString(rs.getString("uuid"))))
                                        current = iConomy.Accounts.get(UUID.fromString(rs.getString("uuid")));
                                    else
                                        iConomy.Accounts.create(UUID.fromString(rs.getString("uuid")), rs.getDouble("balance"));
                                ...
                                if (next)
                                    if (iConomy.Accounts.exists(UUID.fromString(rs.getString("uuid"))))
                                        if (rs.getBoolean("hidden"))
                                            iConomy.Accounts.get(UUID.fromString(rs.getString("uuid"))).setStatus(1);
